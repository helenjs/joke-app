import {JokeDataSingle, JokeFullListData} from '@/pages';
 ...
 * @returns {Promise<JokeFullListData | JokeDataSingle>} - A promise that resolves with the jokes data in JSON format
 * @throws {Error} - Throws an error if the API response is not ok
 */
export const fetchJokes = async (count: number): Promise<JokeFullListData | JokeDataSingle> => {
